...
spark = SparkSession.builder \
    .appName("yyd") \
    .config("spark.driver.extraClassPath", "E:\PycharmeProjects\\flaskProject\lib\mysql-connector-java-8.0.11.jar") \
    .getOrCreate()
...
def conn():
    ...
    # 连接本地spark
    spark = SparkSession .builder .appName("yyd") .getOrCreate()
    ...
    tableDF = spark.read.format("jdbc").option("url","jdbc:mysql://47.120.55.226:3306/exp1").option("driver","com.mysql.cj.jdbc.Driver").option("dbtable", "table1").option("user", "root").option("password", "Yqs0417*").load()
...
def get_articles(tableDF):
    ...
    print(articles_column)
    print(articles_value)
